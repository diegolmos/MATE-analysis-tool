...
{
    TF1* f = new TF1("expoFit", "expo", 0, h.GetXaxis()->GetXmax());
    f->SetLineColor(kRed);
    h.Fit(f, "Q0");                  
    return f;
}
...
static Long64_t bestCandidate(const std::vector<Long64_t>& cands,
                              Long64_t prev)
{
    Long64_t best = -1;
    for (auto c : cands) {
                     ...
                     int    nBins)
{
    ...
    //HISTO DELTA T
    TH1D h("h_dt","Distribucion #Delta t;#Delta t (s);Conteos",
           nBins, 0, histRangeS);
    ...
    while (reader.Next())
    {
        ...
        if (prev_ts > 0) {
            double dt = (ts - prev_ts) * NS2S;
            if (dt > 0 && dt < histRangeS) h.Fill(dt);
        }
        prev_ts = ts;
    }
