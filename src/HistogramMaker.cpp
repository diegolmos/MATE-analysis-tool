...
static TH1D* makeH1(TTree* tree,
                    const std::string& var,
                    const std::string& hname)
{
    auto* h = new TH1D(hname.c_str(),
                       (hname+";Barra;Frecuencia (hits)").c_str(),
                       12, 1, 13);               // 12 bins: 1-12
...
{
    auto* h = new TH2D(hname.c_str(),
                       (p.title+";A;B").c_str(),
                       12, 1, 13, 12, 1, 13);
    tree->Draw((p.B+"[0]:"+p.A+"[0]>>"+hname).c_str(),
               cut.c_str(), "goff");
    return h;
}
