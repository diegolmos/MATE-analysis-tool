...
PathConfig PathConfig::makeInteractive(PathConfig cfg){auto ask = [](const std::string& msg, std::string& val) {
    if (val.empty()) {std::cout << msg << " (enter = .): "; std::getline(std::cin, val); if (val.empty()) val = ".";}};
